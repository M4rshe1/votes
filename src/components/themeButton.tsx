...
export const ThemeButton = ({theme}: { theme: string | Object }) => {

    useEffect(() => {
        themeChange(false)
    }, [])

    theme = typeof theme === 'string' ? theme : Object.keys(theme)[0]

    return (
        <button className="outline-base-content text-start outline-offset-4" data-set-theme={theme} onClick={() => localStorage.setItem('novotes_theme', theme as string)}
            ...
            className="flex-grow text-sm">{theme as string}</span> <span
